...
pub trait GatewayPlugin: Send + Sync {
    ...
    async fn react(
        ...
        Err(GatewayError::NotSupported { operation: "react".to_string() })
    }
    ...
    async fn edit(
        ...
        Err(GatewayError::NotSupported { operation: "edit".to_string() })
    }

    /// Delete message
    async fn delete(&self, message_id: MessageId) -> GatewayResult<()> {
        let _ = message_id;
        Err(GatewayError::NotSupported { operation: "delete".to_string() })
    }
    ...
    async fn typing(
        ...
        let _ = channel;
        Err(GatewayError::NotSupported { operation: "typing".to_string() })
    }
