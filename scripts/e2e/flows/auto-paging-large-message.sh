...
flow_main() {
  ...
  peko_iso_run send sam \
      "Use the Agent tool to delegate this to a helper: run 'yes | head -c 5000' in bash and report the exact output. Don't summarise — I want to see the full text. The helper should not modify it."
  peko_iso_assert_rc_zero
  ...
  # ── assert round-5 F3 fix: rotated id appears in `session list` ─
  echo
  echo "─── POST: session list contains the rotated id (F3 fix) ──────"
  local list_output
  list_output=$(peko_iso_run session list --principal sam)
  local first_chapter
  ...
  if echo "$list_output" | grep -q "$first_chapter"; then
    echo "✓ rotated id $first_chapter appears in session list"
  else
    echo "❌ rotated id $first_chapter missing from session list (F3 regression)"
    return 1
  fi
