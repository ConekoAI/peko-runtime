...
| 22 | Subagent dead code + cleanup branch (PR B4) | Announcement cluster retired end-to-end: `AnnouncementSender`/`Receiver` + `with_announcement_channel`/`create_announcement_channel`/`announcement_tx` field/`get_completed_for_announcement`/`send_announcement`/`announcement_sender` deleted on `SubagentExecutor`; `subagent_announce::format_announcement` deleted (its 2 tests went with it; `build_subagent_system_prompt` / `build_subagent_task_message` retained); `SubagentExecutor::{with_async_framework, cleanup, shutdown, get_run_status}` removed; unreachable `SpawnCleanupPolicy::Delete` branch deleted from the executor (the variant stays for legacy-JSON reads); `_parent_ctx` + `isolated` plumb-through removed from `SubagentExecutor::spawn_and_execute`, `execute_and_wait`, `Agent::spawn_session`, `SessionManager::spawn_session` — the executor always runs in shared-context mode now; `validate_context_parent` reduced to a `resolve_reference` shim (deeper subtree check was unreachable in production — producer always passes the caller's own key); `DynamicSessionKeyProvider` deleted (`get_session_key` had no readers; only `set_session_key` was called once per subagent in `subagent_executor.rs` — now removed too); `Agent::session_key_provider` accessor + 3 constructions deleted; 14+ `spawn_and_execute` test sites updated to drop the `parent_ctx` / `isolated` args; `validate_context_parent_ownership` test rewritten as `validate_context_parent_resolves_path` (the deeper-subtree assertions were unreachable in production) |
| 23 | Channel dead surface (PR B5) | 5 dead `ChannelKind` variants (`Http`/`P2p`/`Webhook`/`FileWatch`/`A2a`) deleted; kept `Cli`/`Hub`/`Cron`/`Trunk` (the `A2a` test at `principal/manager.rs:2584` migrated to `Cli`); `TunnelMessage::InstanceDeregister` payload + variant + catch-all match removed (was never sent); `runtime_direct_endpoint` field deleted from `InstanceAnnouncePayload` + `TunnelHost` trait accessor + `AppState` impl + 3 announce sites + 5 test sites (transport gone — all cross-runtime traffic flows through the tunnel relay); `Agent::resolve_session` / `resolve_default_session` + `SessionManager::route_to_agent` retained for the test suite with B5 doc-comment explaining the scope-out (ChannelType + SessionManager::route still embedded in OverlayType/SessionContext — coordinated refactor tracked for a follow-up); `ChannelError::NotEmpty` doc tightened to note no current adapter raises it (the variant stays — future stricter-semantics sweep); `ChannelSubscriber::spawn` was a false positive (no TODO marker exists in `subscription.rs`); test sites in `tunnel_integration.rs` + `protocol.rs` updated for the dropped field |
| 24 | Manifest shell + workspace-dep-rule cleanup (PR B6) | 16 dead rules in `check_workspace_deps.py` removed — 8 referenced the `peko-extension-host` crate (never extracted from `core/src/extensions/framework/`, the canonical home is `peko-rs/core/`) and 8 referenced `peko-peko-daemon` / `peko-peko-cli` (canonical names are `peko-daemon` / `peko-cli`); `FORBIDDEN_EDGES` shrank from 97 → 81 entries and the header doc-comment was updated; `AgentRegistry::store_manifest` + `set_tag`/`list_tags`/`resolve_tag` + `get_manifest_by_digest`/`get_manifest_by_tag` + `gc` + `referenced_digests` + `manifests_dir`/`tags_dir` + `digest_key` + `GcStats` deleted (the `.agent` TOML machinery had no production consumers — the live path is `RegistryClient::store_manifest_locally` writing `registry_manifests/<digest>/manifest.json`); `AgentRegistry::init()` slimmed to only `layers_dir`; `agent_manifest_digest` helper added so the 3 `registry_integration.rs` callsites compute the same digest the deleted method would have (without the filesystem side-effect); `encode_tag`/`decode_tag` retained — they're consumed by the live `client.rs`; 4 dead tests in `agent_registry.rs::tests` deleted (`test_manifest_store_and_get` / `test_tag_resolution` / `test_encode_tag_*` / `test_gc_removes_unreferenced_layers_only`); `AgentRegistry` now exposes only the layer-API (store/get/has/size + storage limits) |

#### Current crate layout (21 workspace members, 2026-08-23)
