            ...
            Ok(entries) => {
                for entry in entries {
                    let modified = SystemTime::UNIX_EPOCH
                        + Duration::from_millis(entry.updated_at);
    ...
    if json {
        ...
        if all_sessions.is_empty() {
            ...
            for (agent, session_id, session_key, modified, size, msg_count, tokens) in all_sessions {
                ...
                let is_cli_default = session_key.as_ref()
                ...
                let display_id = session_key.as_ref().map(|s| s.as_str()).unwrap_or(&session_id);
...
async fn run_maintenance(
    ...
    for agent_name in agents_to_check {
        ...
        let config = MaintenanceConfig {
            mode: if execute { MaintenanceMode::Auto } else { MaintenanceMode::Warn },
            ..Default::default()
        };
        ...
        match index.maintenance(&config).await {
            Ok(report) => {
                ...
                if report.rotated { total_rotated += 1; }
...
async fn show_session(
    ...
    let session_path = agents_dir.join(&agent_name).join("sessions").join(&entry.transcript_file);
