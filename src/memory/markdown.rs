...
impl MarkdownMemory {
    ...
    pub async fn append_daily(&mut self,
        content: &str,
    ...
    pub async fn write_long_term(
        ...
        info!("Updated long-term memory: {} (section: {})", path.display(), section);
        Ok(())
    }
    ...
    pub async fn read_long_term(&self,
    ) -> Result<Option<String>> {
        let path = self.long_term_path();
    ...
    pub async fn list_daily_files(&self,
    ) -> Result<Vec<PathBuf>> {
        let daily_dir = self.config.workspace_dir.join(&self.config.daily_dir);
...
mod tests {
    ...
    async fn test_daily_file_path() {
        ...
        assert!(path.to_string_lossy().contains(&date.format("%Y-%m-%d").to_string()));
    }
    ...
    async fn test_append_and_read_daily() {
        ...
        memory.append_daily("Test memory entry", None).await.unwrap();
    ...
    async fn test_write_and_read_long_term() {
        ...
        memory.write_long_term("Preferences", "User prefers dark mode").await.unwrap();
        memory.write_long_term("Preferences", "User likes cats").await.unwrap();
