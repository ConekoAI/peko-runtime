...
impl HygieneRunner {
    ...
    pub fn load_state(&self, workspace_dir: &Path) -> Result<HygieneState> {
        ...
        let content =
            std::fs::read_to_string(&state_path).context("Failed to read hygiene state")?;

        let state: HygieneState =
            serde_json::from_str(&content).context("Failed to parse hygiene state")?;
    ...
    pub fn save_state(&self, workspace_dir: &Path, state: &HygieneState) -> Result<()> {
        ...
        std::fs::write(&state_path, content).context("Failed to write hygiene state")?;
