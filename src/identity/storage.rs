...
use crate::identity::did::{Identity, DIDScope};
...
impl KeyStorage {
    ...
    pub fn generate_identity(&self, scope: DIDScope, tenant: Option<&str>) -> Result<Identity> {
        let identity = Identity::generate(scope, tenant)
    ...
    pub fn store(&self, identity: &Identity) -> Result<()> {
        let keypair = identity.keypair.as_ref()
...
mod tests {
    ...
    fn test_list_identities() {
        ...
        storage.generate_identity(DIDScope::Local, Some("acme")).unwrap();
