...
impl Default for TranscriptConfig {
    fn default() -> Self {
        ...
        Self {
            storage_dir: home.join(".pekobot").join("agents").join("main").join("sessions"),
            auto_create: true,
        }
    }
}
...
impl TranscriptStorage {
    ...
    async fn ensure_dir(&self) -> Result<()> {
        ...
        if !self.config.storage_dir.exists() {
            ...
            debug!("Created transcript directory: {}", self.config.storage_dir.display());
        }
    ...
    pub async fn read(
        &self, session_id: &str) -> Result<Vec<TranscriptEntry>> {
        let path = self.transcript_path(session_id);
    ...
    pub async fn search(
        ...
        let matches: Vec<TranscriptEntry> = entries.into_iter()
            .filter(|e| {
    ...
    pub async fn status(&self) -> Result<String> {
        ...
        let total_entries: usize = futures::future::try_join_all(
            transcripts.iter().map(|(_, id)| self.stats(id))
        ).await.map(|stats| stats.iter().map(|s| s.entry_count).sum())?;
...
mod tests {
    ...
    async fn test_multiple_entries() {
        ...
        storage.append("session-2", &TranscriptEntry::system("You are helpful")).await.unwrap();
        storage.append("session-2", &TranscriptEntry::user("Hi")).await.unwrap();
        storage.append("session-2", &TranscriptEntry::assistant("Hello!")).await.unwrap();
    ...
    async fn test_stats() {
        ...
        storage.append("session-3", &TranscriptEntry::user("Test")).await.unwrap();
        storage.append("session-3", &TranscriptEntry::assistant("Response")).await.unwrap();
    ...
    async fn test_delete() {
        ...
        storage.append("session-4", &TranscriptEntry::user("Test")).await.unwrap();
