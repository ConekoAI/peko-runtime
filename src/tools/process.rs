...
impl ProcessTool {
    ...
    async fn execute_command(
        ...
        match result {
            ...
            Ok(Err(e)) => {
                Err(anyhow::anyhow!("Failed to execute command '{}': {}", command, e))
            }
            Err(_) => {
                Err(anyhow::anyhow!("Command '{}' timed out after {} seconds", command, timeout_secs))
            }
        }
    }
...
impl Tool for ProcessTool {
    ...
    async fn execute(&self, params: serde_json::Value) -> Result<serde_json::Value> {
        ...
        if let Some(shell_cmd) = params.get("shell").and_then(|v| v.as_str()) {
            if !self.allow_shell {
                return Err(anyhow::anyhow!("Shell execution not enabled. Use command and args instead."));
            }
...
mod tests {
    ...
    async fn test_process_echo() {
        ...
        assert!(response.get("stdout").unwrap().as_str().unwrap().contains("Hello World"));
    }
    ...
    async fn test_process_shell_enabled() {
        ...
        let response = result.unwrap();
        assert!(response.get("success").unwrap().as_bool().unwrap());
        assert!(response.get("stdout").unwrap().as_str().unwrap().contains("hello_world"));
    }
}
