...
impl MessageTool {
    ...
    async fn send_message(
        ...
        match channel {
            ChannelType::Discord => {
                self.send_discord(content).await
            }
            ChannelType::Slack => {
                self.send_slack(recipient, content).await
            }
            ChannelType::Telegram => {
                self.send_telegram(recipient, content).await
            }
            ChannelType::Whatsapp => {
                self.send_whatsapp(recipient, content).await
            }
            ChannelType::Signal => {
                self.send_signal(recipient, content).await
            }
            ChannelType::Email => {
            ...
            ChannelType::Webhook => {
                self.send_webhook(recipient, content).await
            }
        }
    }
    
    /// Send Discord message via webhook
    async fn send_discord(&self,
        content: &str,
    ) -> Result<MessageResult> {
        let webhook_url = self.config.discord_webhook_url.as_ref()
...
impl Tool for MessageTool {
    ...
    async fn execute(
        ...
        let result = self.send_message(channel, recipient, content, subject).await?;
