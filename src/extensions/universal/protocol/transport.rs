...
pub struct Transport {
    inner: peko_extension_host::protocols::shared::process_transport::ProcessTransport,
...
impl Transport {
    ...
    pub async fn spawn(executable: impl AsRef<std::path::Path>) -> Result<Self> {
        let inner =
            peko_extension_host::protocols::shared::process_transport::ProcessTransport::spawn_default(
                executable,
            )
            .await?;
