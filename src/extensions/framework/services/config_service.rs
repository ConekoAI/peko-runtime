...
//! and agent scopes. Stores data as TOML files under the data directory.
...
pub enum ConfigScope {
    /// Global scope (applies to all agents)
    Global,
...
impl ExtensionConfigData {
    ...
    fn set(&mut self, agent: Option<&str>, key: String, value: serde_json::Value) {
        let target = match agent {
            Some(agent_id) => self.agents.entry(agent_id.to_string()).or_default(),
            None => &mut self.global,
        };
        target.insert(key, value);
    }

    fn unset(&mut self, agent: Option<&str>, key: &str) -> bool {
        match agent {
            Some(agent_id) => {
                if let Some(agent_config) = self.agents.get_mut(agent_id) {
                    agent_config.remove(key).is_some()
                } else {
                    false
                }
            }
            None => self.global.remove(key).is_some(),
        }
    }

    fn get_scope(&self, agent: Option<&str>) -> &HashMap<String, serde_json::Value> {
        match agent {
            Some(agent_id) => self.agents.get(agent_id).unwrap_or(&self.global),
            None => &self.global,
        }
    }
}
...
mod tests {
    ...
    fn test_set_and_show_agent() {
        ...
        service
            .set(
                "test-ext",
                ConfigScope::Agent("myagent".to_string()),
                "timeout",
                serde_json::json!(30),
            )
            .unwrap();
        ...
        let config = service
            .show("test-ext", ConfigScope::Agent("myagent".to_string()))
            .unwrap();
