        ...
        match result {
            ...
            Err(e) => {
                ...
                assert!(
                    ...
                    "Error should mention dimension: {}", e
                );
            }
        }
    }
...
fn test_vector_search_basic() {
    ...
    let entries = vec![
        ("I love machine learning", mock_embed("I love machine learning")),
        ("The weather is nice today", mock_embed("The weather is nice today")),
        ("Rust is a systems programming language", mock_embed("Rust is a systems programming language")),
        ("Deep learning is a subset of ML", mock_embed("Deep learning is a subset of ML")),
        ("It's raining cats and dogs", mock_embed("It's raining cats and dogs")),
    ];
    
    for (content, embedding) in &entries {
        memory.store_with_embedding(content, None, embedding).unwrap();
    }
...
fn test_vector_search_with_metadata_filter() {
    ...
    let results = memory.search_vector_with_filter(
        &query_embedding,
        10,
        0.0,
        json!({"project": "alpha"}),
...
fn test_vector_search_with_very_large_embedding() {
    ...
    let result = memory.store_with_embedding(
        "Content with large embedding",
        None,
        &large_embedding,
    );
...
fn test_vector_search_performance_large_dataset() {
    ...
    for i in 0..10000 {
        let content = format!("Document {} with varied content for semantic search testing", i);
        let embedding = mock_embed(&content);
        memory.store_with_embedding(&content, None, &embedding).unwrap();
    }
    ...
    assert!(!results.is_empty());
    assert!(elapsed.as_millis() < 5000, "Large dataset search took too long: {:?}", elapsed);
}
