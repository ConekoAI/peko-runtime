...
async fn test_agent_manager_creates_all_tools() {
    ...
    let (manager, _receiver) = AgentManager::new().await
    ...
    assert!(tools.len() >= 14, "Expected at least 14 tools, got {}", tools.len());
    ...
    // Core tools
    assert!(tool_names.contains(&"filesystem".to_string()), "Missing filesystem tool");
    assert!(tool_names.contains(&"process".to_string()), "Missing process tool");
    assert!(tool_names.contains(&"http".to_string()), "Missing http tool");
    assert!(tool_names.contains(&"fetch".to_string()), "Missing fetch tool");
    assert!(tool_names.contains(&"web_search".to_string()), "Missing web_search tool");
    assert!(tool_names.contains(&"apply_patch".to_string()), "Missing apply_patch tool");
    assert!(tool_names.contains(&"browser".to_string()), "Missing browser tool");
    
    // Session introspection
    assert!(tool_names.contains(&"sessions_list".to_string()), "Missing sessions_list tool");
    assert!(tool_names.contains(&"sessions_history".to_string()), "Missing sessions_history tool");
    assert!(tool_names.contains(&"session_status".to_string()), "Missing session_status tool");
    assert!(tool_names.contains(&"session_messaging".to_string()), "Missing session_messaging tool");
    
    // Agent management
    assert!(tool_names.contains(&"agents_list".to_string()), "Missing agents_list tool");
    assert!(tool_names.contains(&"agent_info".to_string()), "Missing agent_info tool");
    assert!(tool_names.contains(&"agent_spawn".to_string()), "Missing agent_spawn tool");
    assert!(tool_names.contains(&"agent_broadcast".to_string()), "Missing agent_broadcast tool");
...
async fn test_tool_factory_presets() {
    ...
    assert_eq!(minimal.len(), 2, "Minimal should have 2 tools (filesystem, process)");
...
async fn test_tools_have_descriptions() {
    ...
    for tool in &tools {
        ...
        assert!(!desc.is_empty(), "Tool {} should have description", tool.name());
