...
async fn test_tools_smoke() {
    use pekobot::tools::{
        ApplyPatchTool, BrowserTool, FetchTool, FileSystemTool, HttpTool, ProcessTool,
        WebSearchTool,
    };
    ...
    assert_eq!(
        ApplyPatchTool::new(Default::default(), PathBuf::from(".")).name(),
        "apply_patch"
    );
    ...
    use pekobot::tools::{
        InMemorySessionRegistry, SessionStatusTool, SessionsHistoryTool, SessionsListTool,
    };

    let registry = InMemorySessionRegistry::new("main".to_string());
    assert_eq!(
        SessionsListTool::new(Box::new(registry)).name(),
        "sessions_list"
    );

    let registry2 = InMemorySessionRegistry::new("main".to_string());
    assert_eq!(
        SessionsHistoryTool::new(Box::new(registry2)).name(),
        "sessions_history"
    );

    let registry3 = InMemorySessionRegistry::new("main".to_string());
    assert_eq!(
        SessionStatusTool::new(Box::new(registry3)).name(),
        "session_status"
    );
    ...
    let (tx, _rx) = mpsc::channel(10);
    assert_eq!(
        AgentBroadcastTool::new(tx.clone()).name(),
        "agent_broadcast"
    );
...
async fn test_fetch_tool() {
    use pekobot::tools::{FetchConfig, FetchTool};
...
async fn test_apply_patch_tool() {
    use pekobot::tools::{ApplyPatchConfig, ApplyPatchTool};
