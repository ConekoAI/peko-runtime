...
mod tests {
    ...
    /// Stub store that satisfies the trait port's `get_extension` only —
    /// packaging only reads from the store, never writes.
    struct StubStore {
        exts: Mutex<HashMap<String, LoadedExtension>>,
    }
    ...
    impl StubStore {
        ...
        fn insert(&self, loaded: LoadedExtension) {
            self.exts
                .lock()
                .unwrap()
                .insert(loaded.manifest.id.to_string(), loaded);
        }
    }

    #[async_trait::async_trait]
    impl ExtensionStore for StubStore {
        async fn get_extension(&self, id: &ExtensionId) -> Option<LoadedExtension> {
            self.exts.lock().unwrap().get(&id.to_string()).cloned()
        }

        async fn resolve_tool_name(&self, _name: &str) -> Option<ToolResolution> {
            None
        }

        async fn install(&self, _path: &Path) -> anyhow::Result<ExtensionId> {
            unimplemented!("packaging tests do not exercise install")
        }
    }
    ...
    async fn create_store_with_extension(temp: &TempDir, id: &str) -> (StubStore, PathBuf) {
