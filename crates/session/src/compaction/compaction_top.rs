...
//! lifted type (down from `Option<crate::compaction::summary_format::CompactionDetails>`
//! in root). Callers that need strongly-typed access to details
//! should continue importing `crate::compaction::summary_format::CompactionDetails`
...
impl Compactor {
    ...
    async fn generate_summary_with_llm(
        ...
        // principal + peer scopes auto-charge the summarization call.
        // `StackedMeteredProvider` lives in `peko-engine`; peko-session
        // cannot depend on peko-engine. Phase 7 defers the per-call
        // metering wrapping to the caller (BackgroundCompactor worker
        // opens its own `QuotaScope::with` and wraps the provider
        // before submitting to `Compactor::compact`). Here we just
        // call the provider directly — same behavior as the
        // F18-era `Provider` direct call.
    ...
    pub async fn compact(
        ...
        let summary = if is_split_turn && !to_compact.is_empty() {
            ...
            let turn_prefix = crate::compaction::turn_boundaries::extract_turn_prefix(
                &conversation_msgs,
