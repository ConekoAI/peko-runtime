...
//! in the body are resolved by [`body::preprocess_dynamic_context`]
...
//! LLM to call `Bash` first. See `body` for the shell runner,
...
use crate::skill::body::preprocess_dynamic_context;
use crate::skill::frontmatter::{parse_yaml_frontmatter_typed, SkillFrontmatter};
...
pub const ESCAPE_SENTINEL: &str = "\x00ESCAPED_DOLLAR\x00";
...
impl SkillTool {
    /// Build a `SkillTool` bound to the given runtime port.
    #[must_use]
...
- { error, skill } — structured error: skill_not_enabled, skill_unreadable, or unknown_skill."#
    ...
    async fn execute(&self, params: serde_json::Value) -> anyhow::Result<serde_json::Value> {
        ...
        let ctx = peko_tools_core::ToolContext::default_for_tool("Skill");
    ...
    async fn execute_with_context(
        ...
        let body = preprocess_dynamic_context(body, &frontmatter, &workspace).await?;
...
pub mod default_runtime {
    ...
    impl crate::skill::SkillRuntime for CatalogAdapter {
        fn resolve_skill(&self, name: &str) -> Option<SkillEntry> {
...
mod tests {
    use super::*;
    use crate::skill::tool::default_runtime::InMemorySkillCatalog;
