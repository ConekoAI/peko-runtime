...
| `ExecutionConfig.{conversation, conversation_channel, conversation_peer}` / `AgentConfig.{conversation_channel, conversation_peer}` / `Agent::with_conversation_{channel, peer}` / `AgentView::conversation_{channel, peer}` / `TurnPromptContext.conversation_{channel, peer}` | `agents::subagent_executor`, `agents::agent_config`, `agents::agent`, `peko_engine::agent_view`, `peko_engine::prompt::context` | ✅ New | Conversation mode for peer-ingress turns: skips the subagent wrapper, loads prior session history, and renders the peer's DM channel + subject into the `{{session_context}}` prompt section |
...
### ADR-052 (2026-09-11) — tiered system prompt (D2/D4/D5 prototype slices + D3 fix + D5 T2 project context + D6 workspace-hook prompt sections)
...
| `PrincipalRoutingConfig.peer_agent` / `principal::agent_prompt::resolve_agent_prompt` | `principal::config`, `principal::agent_prompt` | ✅ New | D3: opt-in T1 role for peer-facing turns — names a workspace role file (`agents/<name>.md` / `agents/<name>/AGENT.md`) whose body replaces the root persona in `peer_child_agent_config`; only the prompt body swaps (registry-key invariant kept); unresolvable roles warn + fall back to the root persona |
