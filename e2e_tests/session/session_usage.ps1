...
peko send $agentName "What color is an orange?" 2>&1
...
peko send $agentName "What's USA's Capital?" --new --no-stream 2>&1

Write-Host "`nSending a follow-up message..." -ForegroundColor Cyan
peko send $agentName "What about Canada?" --no-stream 2>&1
...
cat "$env:USERPROFILE/AppData/Roaming/peko/sessions/default/$agentName/$sessionId1.jsonl" | Select-Object -Last 1
Write-Host "`nSession 2 JSONL:" -ForegroundColor Cyan
cat "$env:USERPROFILE/AppData/Roaming/peko/sessions/default/$agentName/$sessionId2.jsonl" | Select-Object -Last 3
...
foreach ($sessionId in @($sessionId1, $sessionId2)) {
    $jsonlPath = "$env:USERPROFILE/AppData/Roaming/peko/sessions/default/$agentName/$sessionId.jsonl"
