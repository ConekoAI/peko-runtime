...
Write-Host "Installing skill from: $skillDir" -ForegroundColor Yellow

$installResult = pekobot cap skill install $skillDir 2>&1
Write-Host $installResult

# Verify installation
$skillList = pekobot cap skill list 2>&1
if ($skillList -match "calculator-skill") {
    Write-Host "✓ Skill 'calculator-skill' installed successfully" -ForegroundColor Green
} else {
    Write-Error "Skill installation failed"
}

# ============================================================
# TEST 2: List and show skill info
# ============================================================
Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "TEST 2: List and show skill info" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan

Write-Host "Skills list (long format):" -ForegroundColor Cyan
pekobot cap skill list --long 2>&1

Write-Host "`nSkill info:" -ForegroundColor Cyan
$infoResult = pekobot cap skill info calculator-skill 2>&1
Write-Host $infoResult

if ($infoResult -match "calculator-skill" -and $infoResult -match "arithmetic") {
    Write-Host "✓ Skill info shows correct details" -ForegroundColor Green
} else {
    Write-Host "⚠ Skill info may be incomplete" -ForegroundColor Yellow
}
...
Write-Host "`nEnabling calculator-skill for $teamName/$agentName..." -ForegroundColor Yellow
pekobot cap enable "$teamName/$agentName" calculator-skill 2>&1 | Out-Null
Write-Host "✓ Skill enabled for agent" -ForegroundColor Green
...
if ($status -match "calculator-skill" -or $status -match "skills") {
    Write-Host "✓ Skill appears in agent status" -ForegroundColor Green
} else {
    Write-Host "⚠ Skill may not be properly enabled" -ForegroundColor Yellow
}
...
# ============================================================
# TEST 6: Test skill read command
# ============================================================
Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "TEST 6: Test skill read command" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan

Write-Host "Reading skill content:" -ForegroundColor Yellow
$readResult = pekobot cap skill read calculator-skill 2>&1
if ($readResult -match "---" -and $readResult -match "name: calculator-skill") {
    Write-Host "✓ Skill read command works correctly" -ForegroundColor Green
} else {
    Write-Host "⚠ Skill read may not work correctly" -ForegroundColor Yellow
}

# ============================================================
# TEST 7: Verify skill appears in cap list
# ============================================================
Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "TEST 7: Skill appears in unified cap list" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan

$capList = pekobot cap list --type skill 2>&1
Write-Host $capList

if ($capList -match "calculator-skill") {
    Write-Host "✓ Skill appears in 'cap list --type skill'" -ForegroundColor Green
} else {
    Write-Host "⚠ Skill not found in cap list" -ForegroundColor Yellow
}
...
# Uninstall skill
pekobot cap skill uninstall calculator-skill --force 2>&1 | Out-Null
Write-Host "Uninstalled calculator-skill" -ForegroundColor Green

Write-Host "`n✅ Skill Capability E2E test completed!" -ForegroundColor Green
Write-Host "" -ForegroundColor Cyan
Write-Host "Summary:" -ForegroundColor Cyan
Write-Host "  - Skill installed from local directory via CLI" -ForegroundColor Cyan
Write-Host "  - Skill listed and info displayed correctly" -ForegroundColor Cyan
Write-Host "  - Skill enabled for agent via 'pekobot cap enable'" -ForegroundColor Cyan
Write-Host "  - Agent successfully used skill via pekobot send" -ForegroundColor Cyan
Write-Host "  - Skill appeared in unified capability list" -ForegroundColor Cyan
Write-Host "" -ForegroundColor Cyan
Write-Host "Architecture verified:" -ForegroundColor Cyan
Write-Host "  - Skill CLI commands work (list, install, uninstall, info, read)" -ForegroundColor Cyan
Write-Host "  - Skill enable/disable integrates with agent config" -ForegroundColor Cyan
Write-Host "  - Skills appear in system prompt for LLM" -ForegroundColor Cyan
Write-Host "  - Unified capability framework includes skills" -ForegroundColor Cyan
