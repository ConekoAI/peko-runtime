...
$result = peko send $agentName 'Use your str_replace_file tool (NOT shell) to modify config.txt. Set old_string=`name = "Original Name"` and new_string=`name = "New Name"`. The old string must match exactly.' --no-stream 2>&1
...
$result = peko send $agentName 'Use your str_replace_file tool (NOT shell) to modify config.txt. Make TWO replacements: 1) old_string=`version = "1.0.0"`, new_string=`version = "2.0.0"` 2) old_string=`debug = true`, new_string=`debug = false`.' --no-stream 2>&1
Write-Host "Response: $result"
...
$result = peko send $agentName "Use your str_replace_file tool (NOT shell) to modify config.txt. Try to replace old_string='NONEXISTENT_STRING' with new_string='something'. This should fail since the string doesn't exist." --no-stream 2>&1
Write-Host "Response: $result"
