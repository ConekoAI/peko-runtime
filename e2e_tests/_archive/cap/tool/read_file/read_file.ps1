...
$result = peko send $agentName "Use your read_file tool (not shell) to read the file called 'test_read.txt' in your workspace. Report exactly what the read_file tool returns." --no-stream 2>&1
...
$result = peko send $agentName "Use your read_file tool with line_range parameter set to '2-3' to read only lines 2-3 from 'test_read.txt'. Tell me exactly what those lines contain." --no-stream 2>&1
Write-Host "Response: $result"
