...
#
# Tests the task_status and task_list tools:
# - task_status: check the status of any async task by task_id
# - task_list: list all async tasks for the current session
#
...
Write-Host "Universal Task Management — Status & List E2E Test" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
try {
    ...
    # ============================================================
    # TEST 1: task_status returns correct status
    # ============================================================
    Write-Host "`n========================================" -ForegroundColor Cyan
    Write-Host "TEST 1: task_status tool" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan

    $statusFile = "status_test.txt"
    $prompt = 'Use agent_spawn with _async=true to delegate this task: Use shell to run: Start-Sleep 8; echo STATUS_TEST_DONE > ' + $statusFile + '. You will get a receipt. Extract the run_id (task_id) from the receipt and use task_status with that task_id to check the status. If status is running or pending, respond with STATUS_RUNNING_OK. If completed immediately, respond with STATUS_ALREADY_DONE. If the tool fails, respond with STATUS_TOOL_FAILED.'
    ...
    if ($response -match "STATUS_RUNNING_OK") {
        Write-Host "PASS: task_status correctly showed running/pending status" -ForegroundColor Green
        ...
        Write-Host "FAIL: task_status tool unavailable or failed" -ForegroundColor Red
        $script:failed = $true
    } else {
    ...
    # ============================================================
    # TEST 2: task_status shows completed after task finishes
    # ============================================================
    Write-Host "`n========================================" -ForegroundColor Cyan
    Write-Host "TEST 2: task_status shows completed after finish" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan

    Write-Host "Waiting for async task to complete (8s)..." -ForegroundColor Yellow
    Start-Sleep 8

    $prompt2 = 'Use the task_status tool again with the same task_id (run_id) from the previous task. Also check if the file ' + $statusFile + ' exists. If task_status shows status completed and the file exists with STATUS_TEST_DONE, respond with STATUS_COMPLETE_OK. If status is still running, respond with STATUS_STILL_RUNNING. If status shows failed or timed_out, respond with STATUS_COMPLETE_FAILED. If the tool is unavailable, respond with STATUS_TOOL_MISSING.'
    ...
    if ($response2 -match "STATUS_COMPLETE_OK" -and $fileExists -and $fileContent -match "STATUS_TEST_DONE") {
        Write-Host "PASS: task_status showed completed and file was created" -ForegroundColor Green
        ...
        Write-Host "FAIL: task_status tool not available" -ForegroundColor Red
        $script:failed = $true
    } else {
        Write-Host "Result unclear - manual review needed" -ForegroundColor Yellow
    }
    ...
    # ============================================================
    # TEST 3: task_list shows active tasks
    # ============================================================
    Write-Host "`n========================================" -ForegroundColor Cyan
    Write-Host "TEST 3: task_list tool" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan

    $prompt3 = 'Launch TWO async subagents concurrently: 1. Use shell to run: Start-Sleep 20; echo done1. 2. Use shell to run: Start-Sleep 20; echo done2. Immediately after getting both receipts, use the task_list tool to list all active tasks. If task_list shows at least 2 active tasks, respond with LIST_OK and the count. If it shows fewer than 2, respond with LIST_LOW_COUNT. If the tool is unavailable, respond with LIST_TOOL_MISSING. If something else goes wrong, respond with LIST_FAILED.'
    ...
    if ($response3 -match "LIST_OK") {
        Write-Host "PASS: task_list showed multiple active tasks" -ForegroundColor Green
    } elseif ($response3 -match "LIST_LOW_COUNT") {
        Write-Host "FAIL: task_list showed fewer tasks than expected" -ForegroundColor Red
        $script:failed = $true
    } elseif ($response3 -match "LIST_TOOL_MISSING") {
        Write-Host "FAIL: task_list tool not available" -ForegroundColor Red
        $script:failed = $true
    ...
    # Wait for the 20s tasks to finish before cleanup
...
Write-Host "`nUniversal task status & list e2e tests completed!" -ForegroundColor Green
