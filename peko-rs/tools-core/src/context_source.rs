...
//! the canonical adapter (`ToolContextAdapter`) lives in `tools::core::exec`.
//! The old location at `extensions::framework::protocols::shared::context_resolver`
//! keeps a re-export for one commit while consumers migrate.
