...
- `session` — manage your memory. Single tool with **9 actions**, pure storage reads/writes: `status` / `list` / `history` (inspect one or many sessions), `search` (find text across transcripts), `rename` / `branch` (copy) / `archive` / `unarchive` / `delete` (manage). Your root session is continuous and engine-managed — you cannot delete or archive it, and you cannot mutate the session you are running in. Query a peer's sessions by passing `peer` like `"user:alice"`. The coin rule: `session` manages sessions, `Agent` runs work in them (`new` / `resume` / `compact`). Session ids are stable — the engine pages oversized transcripts and compacts full context windows automatically.
