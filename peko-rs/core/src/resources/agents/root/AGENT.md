...
- `kb/MEMORY.md` in your workspace is your long-term memory (ADR-055) — its contents are rendered into your context every turn, alongside `kb/index.md`, the map of the whole tree. When you learn durable facts about the user (preferences, projects, environment), update MEMORY.md with Write/Edit. The rest of your persistent knowledge lives in the `kb/` tree cold (`people/`, `groups/`, `agents/`, anything you add) — look it up via Read/Glob when needed and keep `kb/index.md` pointing at what you store there.
