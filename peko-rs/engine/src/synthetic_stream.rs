...
//!
//! Lifted from `src/providers/synthetic_stream.rs` in Phase 9b.N.5b.5 —
//! the helper lives next to its consumer (the agentic loop, soon to
//! arrive here) rather than in `peko-provider-api`, because:
//!
//! 1. The provider-API crate is a thin contract layer (per
//!    `crates/provider-api/src/lib.rs`); a runtime helper that uses
//!    `futures::stream::iter` is implementation, not contract.
//! 2. `peko-engine` already brings `futures` transitively via
//!    `peko-extension-host`; promoting it to a direct dep is
//!    zero incremental cost.
//!
//! Once Phase 9b.N.5b.8 moves `agentic_loop.rs` into this crate, the
//! helper and its single consumer will live side by side.
...
mod tests {
    use super::*;
    use peko_provider_api::{StopReason, TokenUsage};
