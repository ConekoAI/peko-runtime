...
peko ext disable shell --target default/$agentName 2>&1 | Out-Null
...
$response2 = peko send $agentName "Try to use the shell tool to list files. If you don't have shell tool access, reply exactly SHELL_BLOCKED, otherwise reply SHELL_SUCCESS" 2>&1
...
peko ext disable glob --target default/$agentName 2>&1 | Out-Null
...
$response3 = peko send $agentName "Your tool access has been updated. Try to use the shell tool to list files in your workspace. If you don't have shell tool access, reply exactly SHELL_BLOCKED, otherwise reply SHELL_SUCCESS" 2>&1
...
$response4 = peko send $agentName "Try to use the glob tool to list files. If glob is not available, reply exactly: GLOB_BLOCKED, otherwise reply GLOB_SUCCESS" 2>&1
