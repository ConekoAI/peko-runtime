...
use pekobot::tools::{
    CalendarCredentials, CalendarProvider, CalendarTool, EmailConfig, EmailCredentials,
    EmailProvider, EmailTool, Tool,
};
...
async fn run_test_scenario(title: &str, recipient: &str) -> anyhow::Result<()> {
    ...
    println!(
        "   ✅ Event created: {}",
        event_details.get("id").unwrap_or(&"unknown".to_string())
    );
    println!("      Title: {}", title);
    ...
    println!(
        ...
        email_result
            .get("message_id")
            .unwrap_or(&"mock-msg-id".to_string())
    );
    println!();
...
async fn create_event(
    ...
    details.insert(
        "id".to_string(),
        format!("evt_{}", uuid::Uuid::new_v4().to_string()[..8].to_string()),
    );
...
async fn send_invite(
    ...
    result.insert(
        "event_id".to_string(),
        event.get("id").unwrap_or(&"unknown".to_string()).clone(),
    );
    Ok(result)
}
