...
async fn main() -> anyhow::Result<()> {
    ...
    let unified = UnifiedRegistry::new(
        pekobot::a2a::registry::create_registry().0,
        client,
    );
    ...
    match unified.search_by_capability("messaging", pekobot::coneko::registry::CapabilityMatch::Any).await {
        Ok(results) => {
            println!("   Found {} agents\n", results.len());
        }
