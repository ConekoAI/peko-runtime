...
async fn test_cron() {
    ...
    // Add some jobs
    let job1 = scheduler.add_job("*/5 * * * *", "echo 'Every 5 minutes'").unwrap();
    let job2 = scheduler.add_job("0 9 * * *", "echo 'Daily at 9am'").unwrap();
    
    println!("✓ Added job 1: {} ({})", job1.id[..8].to_string(), job1.expression);
    println!("✓ Added job 2: {} ({})", job2.id[..8].to_string(), job2.expression);
...
async fn test_heartbeat() {
    ...
    HeartbeatEngine::ensure_heartbeat_file(&new_dir).await.unwrap();
